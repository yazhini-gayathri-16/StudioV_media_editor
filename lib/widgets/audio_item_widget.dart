...
class AudioTimelineWidget extends StatefulWidget {
  ...
  final Function(Duration dragDelta) onPositionChanged; // Callback for when the clip is moved
...
class _AudioTimelineWidgetState extends State<AudioTimelineWidget> {
  ...
  void _handleMiddleDrag(LongPressMoveUpdateDetails details) {
    // UPDATED: Use details.delta.dx for incremental movement
    final deltaMilliseconds = (details.delta.dx * 1000 / widget.pixelsPerSecond).round();
    widget.onPositionChanged(Duration(milliseconds: deltaMilliseconds));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      ...
      // UPDATED: The main GestureDetector now handles long press dragging
      child: GestureDetector(
        onTap: widget.onTap,
        // Only allow long press move if selected
        onLongPressMoveUpdate: widget.isSelected ? _handleMiddleDrag : null,
        child: Container(
          width: width.clamp(60.0, double.infinity), // Ensure minimum width
          height: 40,
...
extension on LongPressMoveUpdateDetails {
  get delta => null;
}